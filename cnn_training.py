...
def get_ResNetMulticlass(dropout, no_classes, no_layers=18, pretrained=False):
    '''
        This function returns a ResNet model with given numbers of layers.
        It adjusts a model for binary classification problem, adds a Dropout layer
        before the output and modifies input layer because our data
        ...
        @param dropout: value of probability p of an element
        to be zeroed in Dropout layer.
        @param no_classes: number of output classes ResNet.
        @param no_layers: number of layers in ResNet. It allows 18, 34 and 50
        layers wariants of ResNet architecture.
        @param pretrained: a bool value if we want to use an Imagenet pretrained weights.
        @return resnet: a ResNet class object.
...
def early_stopping(train_loss, validation_loss, min_delta, counter, tolerance):
...
def train_model(log_file, model, dataloaders, dataset_sizes, criterion, optimizer, scheduler, num_epochs, mixup, alpha):
    ...
    return model, best_auc, best_epoch, epoch_training_time

def kFoldTraining(log_file, no_folds, writer, y, train_dataset, test_dataloader, test_indices, class_weights, hparams, no_classes):
    '''
    ...
    conf_mtx_path = "results/{}_lr{}_d{}_wd{}_conf_mtx".format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S"), hparams["lr"], hparams["Dropout"], hparams["weight_decay"])
    model_path = "models/{}_lr{}_d{}_wd{}_models".format(datetime.datetime.now().strftime("%Y%m%d-%H%M%S"), hparams["lr"], hparams["Dropout"], hparams["weight_decay"])
    output_dir = Path(conf_mtx_path)
    ...
    for fold, (train_ids, val_ids) in enumerate(skf.split(np.zeros(len(y)), y)):
        ...
        log_file.write('--------------------------------\n')
        # Get the model
        model = get_ResNetMulticlass(hparams["Dropout"], no_classes, no_layers=hparams["no_layers"], pretrained=hparams["pretrained"])
        ...
        # train
        fold_model, val_auc, model_epoch, epoch_training_time  = train_model(log_file,
                                                                    model,
                                                                    dataloaders,
                                                                    dataset_sizes,
                                                                    loss_fn,
        ...
        # test
        test_loss, test_auc = test(log_file, test_dataloader, len(test_indices), fold_model, nn.CrossEntropyLoss(), conf_mtx_path, fold)
        torch.save(fold_model.state_dict(), os.path.join(model_path, "ResNet18_multiclass_biopsy_fold_{}.pth".format(fold)))



        # if test_auc > best_test_auc:
        #     best_model = copy.deepcopy(fold_model.state_dict())
    ...
    log_file.write(f'Training epoch complete in {epoch_training_time // 60:.0f}m {epoch_training_time % 60:.0f}s')



def test(log_file, dataloader, size, model, criterion, conf_mtx_path, fold):
    '''
    ...
    conf_mtx = ConfusionMatrixDisplay.from_predictions(all_labels, all_preds)


    output_path = os.path.join(conf_mtx_path, 'Fold{}_confusion_matrix.png'.format(fold))
    conf_mtx.figure_.savefig(output_path)
